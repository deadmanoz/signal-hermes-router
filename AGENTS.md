...
- **Tool-surface completeness must be explicit.** Permission preflight accepts only schema version 1 surfaces with `scope: full_callable`. Unversioned, model-facing, unsupported, or ambiguous surfaces are probe errors and must never be compared as complete catalogs. The producer must include tools deferred behind progressive disclosure.
