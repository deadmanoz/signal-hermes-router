...
- **Tool-surface completeness must be explicit, with one source-aware exception.** Permission preflight accepts only schema version 1 surfaces with `scope: full_callable`. Unversioned, model-facing, unsupported, or ambiguous surfaces from `agentCapabilities._meta`, external contract files, or generic input are probe errors and must never be compared as complete catalogs. The one exception is the Hermes-native `{tools: [...]}` shape returned by the dedicated `_tool_surface/list` JSON-RPC extension method, which the router normalizes into the version-1 `full_callable` contract because the method name itself is the callable-catalog contract. The producer must include tools deferred behind progressive disclosure.
...
Release Please PRs are generated release artefacts when all of the following are true: the head branch is exactly `release-please--branches--main`, the title starts with `chore(main): release`, the diff modifies exactly `CHANGELOG.md`, `.release-please-manifest.json`, `pyproject.toml`, and `uv.lock`, `main` still matches the workflow's triggering SHA, and `scripts/check-release-head.py` confirms the expected version and changelog transformations against that SHA. Once the release workflow has validated that generated head, do not publish implementation findings about those generated contents.
