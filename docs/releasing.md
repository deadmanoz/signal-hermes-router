...
3. The release workflow synchronizes a stale release branch, checks its diff through the GitHub API, and rejects every path except `CHANGELOG.md`, `.release-please-manifest.json`, `pyproject.toml`, and `uv.lock` before checkout. It then checks out the inspected SHA without persisted credentials and runs nonmutating validation with `uv lock --check`, the stdlib-only public-boundary scanner without dependency sync, and a clean-tree check for all generated release files.
