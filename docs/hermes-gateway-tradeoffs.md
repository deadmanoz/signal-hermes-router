...
- **Stable ACP boundary, Hermes-only supervisor today.** The router speaks ACP over stdio, so the profile side only has to implement the ACP server contract - but the shipped supervisor offers no operator-facing way to run anything else: it constructs `hermes -p <profile> acp`, and route config names a Hermes profile. Pointing the router at a non-Hermes ACP agent is a code change, not a configuration. The multiplexing gateway is Hermes-only by design, in-process.
