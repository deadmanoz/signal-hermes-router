...
- **Route-scoped event dedupe** in a sqlite store: turns the router completed (`handled`) can't be re-played across a router restart. Claims left `processing` by a crash mid-turn are reclaimed at the next startup (the router owns the state DB exclusively, so no turn is in flight then), so crash-interrupted turns are at-least-once rather than exactly-once - a deliberate trade against silently dropping their retries.
