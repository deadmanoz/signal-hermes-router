...
Media normally flows inbound: Signal attachments are normalised, stored on disk, and forwarded to Hermes as ACP content blocks. Configured `notify-route` notifications may also carry one trusted local image path with `--attachment`; the router validates that path under `router.media_root` and sends it through signal-cli with the first reply chunk. Route delivery, chunking, retries, and audit logging stay with the router; profiles do not call Signal directly.
